...
<form id="frm" name="frm" enctype="multipart/form-data">
    ...
    <input type="hidden" name="file_gubun" id="file_gubun" value='<c:out value="${searchVO.file_gubun}"/>' />
    <input type="hidden" name="file_id" id="file_id" value='<c:out value="${searchVO.file_id}"/>' />
...
$(function() {
	...
	$("a[name='file']").click(function(){
		...
		//$("#pathurl").val(pathurl);
		$("#file_gubun").val(ext_id);
		$("#file_id").val(content_id);
		if (ext_id == "pdf"){
			$('#my_frame').attr('src', "/cop/bbs/pdfFileView.do?searchAtchFileId="+content_id);
		}
		else if ((ext_id == "png") ||(ext_id == "gif") || (ext_id == "jpg") || (ext_id == "jpeg")){
			$('#my_frame').attr('src', "/cop/bbs/imgView.do?atchFileId="+content_id);
		}
		else{
			$('#my_frame').attr('src', "/display.do?searchAtchFileId="+content_id);
		}
...
function fn_fileDel(atchFileId) {
    ...
    if (window.confirm("파일을 삭제하시겠습니까?")) {
        $("#frm").ajaxForm({
            type: 'POST',
            url: '<c:url value='/cop/bbs/deleteFileSap.do'/>',
            dataType: "json",
            ...
            success: function(result) {
            	...
            	if (result.status == 0) {
                    ...
                    if (result.filecnt == 0) {
                    	...
                    	document.frm.action = "/cop/bbs/selectFileSap.do";
