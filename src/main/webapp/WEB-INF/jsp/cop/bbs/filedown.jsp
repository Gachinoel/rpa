...
<%
    ...
    String realname = ""; 
    // 파일 업로드된 경로
    if (request.getAttribute("filepath") == null) filepath = "";
    else
        filepath = request.getAttribute("filepath").toString();

    // 서버에 실제 저장된 파일명
    if (request.getAttribute("filename") == null) filename = "";
    else
        filename = request.getAttribute("filename").toString();

 // 실제 내보낼 파일명
    if (request.getAttribute("realname") == null) realname = "";
    else
        realname = request.getAttribute("realname").toString();
